"use client";
// FakeMap.tsx — 스타일된 가짜 카카오맵 (API 키 불필요)
// 도로망·공원·하천·마커·경로 폴리라인·지오펜스 원을 표현. 좌표는 0..100 백분율.
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden", background: land, ...style }}>
      ...
      >
        ...
        {/* river */}
        <path
          d="M-5 70 Q 25 60 45 74 Q 65 88 105 78 L 105 100 L -5 100 Z"
          fill={water}
          opacity="0.92"
        />
        <path
          d="M-5 70 Q 25 60 45 74 Q 65 88 105 78"
          fill="none"
          stroke={grey ? "#C7C7C7" : "#8FC0E6"}
          strokeWidth="0.6"
        />
        {/* block fills */}
        ...
        {/* geofence circle */}
        {circle && (
          <g className={circle.tone === "danger" ? "animate-flash-red" : ""}>
            <circle
              cx={circle.x}
              cy={circle.y}
              r={circle.r}
              fill={circle.tone === "danger" ? "rgba(211,47,47,0.15)" : "rgba(46,125,50,0.15)"}
              stroke={circle.tone === "danger" ? "#D32F2F" : "#2E7D32"}
              strokeWidth="0.7"
      ...
      {dim && (
        <div style={{ position: "absolute", inset: 0, background: "rgba(120,124,120,0.32)" }} />
      )}
      ...
      {markers.map((m, i) => (
        <div
          ...
          style={{
            ...
            zIndex: m.active ? 30 : 20,
          }}
        >
          <div
            className={m.active ? "animate-marker-drop" : ""}
            ...
            {m.label && (
              <div
                style={{
                  ...
                  fontWeight: 700,
                  color: m.stale ? "var(--g500)" : "var(--g900)",
                }}
              >
                {m.label}
              </div>
              ...
              >
                <span
                  style={{
                    transform: "rotate(-45deg)",
                    fontSize: 22,
                    filter: m.stale ? "grayscale(1)" : "none",
                  }}
                >
                  {m.emoji}
                </span>
          ...
          {["plus", "minus"].map((k, i) => (
            <div
              ...
              style={{
                ...
                alignItems: "center",
                justifyContent: "center",
                borderBottom: i === 0 ? "1px solid var(--g200)" : "none",
                color: "var(--g700)",
                fontSize: 22,
                fontWeight: 600,
              }}
