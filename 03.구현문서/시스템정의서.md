...
| 모듈명 | 역할 | 관련 기능 ID | 의존성 |
|--------|------|-------------|--------|
| auth | 인증·세션 관리 (회원가입, 로그인, 구글 소셜 연동, 에러 배너 UI 처리) | F-001, F-002 | users 테이블, JWT, Context API |
| groups | 가족 그룹 생성·참여·조회, QR 코드 발급, 초대 링크(`/join?code=`) 자동 가입 연동, 부모님 호칭(관계) 지정 | F-003, F-004, F-030 | groups, group_members 테이블 |
...
| location | 위치 데이터 수신·저장·조회, 역지오코딩 래퍼, 다중 부모님 동시 위치 조회(플랜 한도) | F-009, F-010, F-012, F-013, F-029 | locations 테이블, 카카오맵 API |
...
│   │   │   ├── join/
│   │   │   │   └── page.tsx             # 초대 링크 자동 가입 진입점 (F-004)
│   │   │   ├── onboarding/
...
| v1.1 | 2026-06-08 | PM | 초대 링크(`/join`) 디렉토리 추가 및 모듈 설명 최신화 |
| v1.2 | 2026-06-08 | PM | 프론트엔드 구현 동기화 — 다중 부모님 위치(`current/[groupId]/parents`)·호칭 지정(`groups/[id]/members`) 라우트 및 모듈(F-029, F-030) 반영 |
